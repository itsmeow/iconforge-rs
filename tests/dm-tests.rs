...
/**
 * Find the iconforge_rs binary and DMSRC and copy them into the test run directory
 */
fn find_and_copy_iconforge_rs_lib() -> (String, &'static str, &'static str) {
    ...
    match fs::exists(Path::new(&iconforge_rs_lib_source_path)) {
        Ok(exists) => {
            if !exists {
                panic!(
                    "Source ICONFORGE_RS path does not exist! Try rebuilding the project with the corresponding target and debug or release mode."
                )
            }
        }
