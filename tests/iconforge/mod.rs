...
fn iconforge() {
    ...
    for entry in read_dir("tests/dm/tmp/").unwrap().flatten() {
        if let Some(file_name) = entry.file_name().to_str() {
            ...
            if !std::fs::exists(rust_path).unwrap() {
                panic!(
                    "Could not find corresponding iconforge_rust_{size}.dmi for iconforge_dm_{size}.dmi!"
                )
            }
    ...
    for entry in read_dir("tests/dm/tmp/").unwrap().flatten() {
        if let Some(file_name) = entry.file_name().to_str() {
            ...
            if !std::fs::exists(headless_path).unwrap() {
                panic!(
                    "Could not find corresponding headless_iconforge_rust_{size}.dmi for iconforge_rust_{size}.dmi!"
                )
            }
    ...
    if let Some(diff) = compare_dmis(
        ...
        differences.push(format!(
            "icon tests/dm/tmp/iconforge_gags_rust.dmi differs from tests/dm/rsc/iconforge_gags_dm.dmi:\n{diff}"
        ));
    }
...
fn compare_dmis(dm_path: &Path, rust_path: &Path) -> Option<String> {
    ...
    for rust_state in &rust_icon.states {
        if let None = dm_icon
            .states
            .iter()
            .find(|dm_state| dm_state.name == rust_state.name)
        {
            differences.lock().unwrap().push(format!(
