...
pub fn blend_images_other_universal(
	...
	// Vecs at all times, but this assumption might be incorrect.
	if expected_length_first != image_data.images.len() as u32 {
		return Err(Error::IconForge(format!(
			"Error during blend_images_other - the base set of images did not contain the correct \
			 amount of images (contains {}, it should contain {expected_length_first}) to match \
			 the amount of dirs ({}) or frames ({}) from the first icon state. This shouldn't \
			 ever happen!",
	...
	if expected_length_last != image_data_other.images.len() as u32 {
		return Err(Error::IconForge(format!(
			"Error during blend_images_other - the blending set of images did not contain the \
			 correct amount of images (contains {}, it should contain {expected_length_last}) to \
			 match the amount of dirs ({}) or frames ({}) from the last icon state. This \
	...
	if image_data.dirs != image_data_other.dirs {
		// We can handle the specific case where there's only one dir being blended onto
		// multiple. Copy the icon for each frame onto all dirs.
		if image_data.dirs > image_data_other.dirs && image_data_other.dirs == 1 {
			// Loop backwards so that the frame indexes remain consistent while we iterate,
			// since inserts shift the array right
			...
			return Err(Error::IconForge(format!(
				"Attempted to blend two icon states with different dir amounts with {} and {} \
				 dirs respectively.",
	...
	if image_data.frames != image_data_other.frames {
		// We can handle the specific case where there's only one frame on the base and
		// the other has more frames. Simply add copies of that first frame.
		if image_data_other.frames > 1 && image_data.frames == 1 {
			...
			// Update the output IconState's frame count, because the values from the first
			...
			return Err(Error::IconForge(format!(
				"Attempted to blend two icon states with different frame amounts - with {} and {} \
				 frames respectively.",
...
pub fn blend_images_other(
	...
	if base_icon_state.dirs != blending_icon_state.dirs {
		...
		if base_icon_state.dirs > blending_icon_state.dirs && blending_icon_state.dirs == 1 {
			...
			return Err(Error::IconForge(format!(
				"Attempted to blend two icon states with different dir amounts - {} and {}, with \
	...
	if base_icon_state.frames != blending_icon_state.frames {
		...
		if blending_icon_state.frames > 1 && base_icon_state.frames == 1 {
			...
			return Err(Error::IconForge(format!(
				"Attempted to blend two icon states with different frame amounts - {} and {}, \
				 with {} and {} frames respectively.",
				base_icon_state.name,
				blending_icon_state.name,
				base_icon_state.frames,
				blending_icon_state.frames
			...
			Transform::Flip { dir } => {
				let dir = match dmi::dirs::Dirs::from_bits(*dir) {
					Some(dir) => {
						...
						{
							return Err(format!(
								"Non-square icons cannot be flipped diagonally (Turned&Flipped)! \
								 (used Flip(dir={dir}) on {}x{})",
								image.width(),
								image.height()
							));
						}
