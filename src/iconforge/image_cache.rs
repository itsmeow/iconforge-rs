...
impl UniversalIcon {
	...
	pub fn get_image_data(
		...
		if cached {
			...
			if must_be_cached {
				return Err(format!(
					"Image was requested but does not exist in the cache. It's likely that the \
					 icon state doesn't exist: {self} - while generating '{sprite_name}'"
				));
			}
		}
			...
			if dir_bits > 0 {
				...
				dir_index = match Dirs::from_bits(dir_bits) {
					Some(dir) => {
						...
						{
							return Err(format!(
								"Dir specified {dir} is not in the set of valid dirs ({} dirs) \
								 for icon_state \"{}\" for {sprite_name}",
		...
		if let Some(frame) = self.frame {
			...
			if frame > 0 {
				...
				if state.frames < frame {
					return Err(format!(
						"Specified frame \"{frame}\" is larger than the number of frames ({}) for \
						 icon_state \"{}\" in sprite \"{sprite_name}\"",
		...
		// Don't insert into the cache here, because the cache should only contain
		// transformed images.
		Ok((result, false))
	}
}
