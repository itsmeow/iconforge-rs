...
use crate::{error::Error, iconforge::image_cache::cache_transformed_images};
    ...
    if generate_dmi {
        zone!("headless_write_dmi");
        {
            ...
            if let Err(err) = std::fs::create_dir_all(path.parent().unwrap()) {
                return headless_error(
                    format!(
                        "Error creating output file directories for path '{file_path}' during headless generation: {}",
                        err
                    ),
                    Some(&error.lock().unwrap()),
                );
            ...
            };
            {
                ...
                if let Err(err) = dmi_icon.save(&mut output_file) {
                    return headless_error(
                        format!(
                            "Error saving DMI for file path '{file_path}' during headless generation: {}",
                            err
                        ),
                        Some(&error.lock().unwrap()),
                    );
