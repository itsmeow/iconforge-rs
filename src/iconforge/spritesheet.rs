...
/// This is used to save time decoding 'sprites' a second time between the cache
/// step and the generate step.
	...
	)>;
	{
		...
		sprites_data = sprites_map
			...
			.filter_map(|(sprite_name, icon)| {
				...
				let image_data = match icon.get_image_data(sprite_name, true, false, flatten) {
					Ok((image_data, cached)) => {
						let mut image_data = image_data;
						if !cached {
...
pub fn generate_spritesheet(
...

	{
		...
		// same set of transforms, but change 1 or 2 things at the end. We can greatly
		// reduce the amount of RgbaImages created by first finding these.
		...
		tree_vec.par_iter().for_each(|icons| {
			...
			let unique_icons = DashSet::<&UniversalIcon>::with_capacity(icons.len());
			{
				...
				icons.iter().for_each(|(_, icon)| {
					// This will ensure we only map unique transform sets. This also means each
					// UniversalIcon is guaranteed a unique icon_hash Since all icons share the
					// same 'base'. Also check to see if the icon is already cached. If so, we
					// can ignore this transform chain.
	...
	sprites_map.par_iter().for_each(|sprite_entry| {
		...
		let (image_data, _) = match icon.get_image_data(sprite_name, true, true, flatten) {
			Ok(image) => image,
			Err(err) => {
				error.lock().unwrap().push(err);
				return;
		...
		let first = match image_data.images.first() {
			...
			None => {
				error.lock().unwrap().push(format!(
					"No images contained in output data for \"{sprite_name}\"! This shouldn't \
					 happen..."
				));
				return;
			}
...
fn create_png_image(
	...
	for (idx, sprite_entry) in sprite_entries.iter().enumerate() {
		...
		if image_data.images.len() > 1 {
			return Err(format!(
				"More than one image (non-flattened) sprite {sprite_name} in PNG spritesheet for \
				 icon {icon}!"
			));
		}
...
fn create_dmi_output_states(
	...
	// Sort the output states in the relative order of their existence in the input
	// sprites object. This is important for consistency with DM behavior, and it
	// allows the outputted DMI to be used in IconForge's own cache - they will
	// output in the same order between runs. PNGs don't need these because they're
	// only usable in the UI, but these DMI icons are potentially persistent (they
...
/// recursively applies transforms in a tree structure. Maximum transform depth
/// is 128.
fn transform_leaves(
...
pub fn cache_valid(
	...
	zone!("cache_valid");
	let sprites_hash = fixed_twox_string(sprites_in);
	...
	if dmis.len() > dmi_hashes.len() {
		return Ok(serde_json::to_string::<CacheResult>(&CacheResult {
			result: String::from("0"),
			fail_reason: format!(
				"Input hash matched, but more DMIs exist than DMI hashes provided ({} DMIs, {} \
				 DMI hashes).",
				dmis.len(),
				dmi_hashes.len()
			),
	...
	let fail_reason: Arc<RwLock<Option<String>>> = Arc::new(RwLock::new(None));
	{
		...
		dmis.into_par_iter().for_each(|dmi_path| {
			...
			match dmi_hashes.get(&dmi_path) {
				Some(hash) => {
					zone!("hash_dmi");
					match fixed_twox_file(&dmi_path) {
						Ok(new_hash) => {
							zone!("check_match");
							if new_hash != *hash {
								if fail_reason.read().unwrap().is_some() {
								...
								*fail_reason.write().unwrap() = Some(format!(
									"Input hash matched, but dmi_hash was invalid DMI: dmi_path \
									 (stored hash: {}, new hash: {new_hash})",
									hash.clone()
								));
							}
						}
						Err(err) => {
							...
							*fail_reason.write().unwrap() = Some(format!(
								"ERROR: Error while hashing dmi_path '{dmi_path}': {err}"
							));
						}
					}
				}
				None => {
					...
					*fail_reason.write().unwrap() = Some(format!(
						"Input hash matched, but no dmi_hash existed for DMI: '{dmi_path}'"
					));
				}
			}
		});
